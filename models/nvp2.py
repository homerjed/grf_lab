...
class LinearCouplingLayer(nn.Module):
    ...
    def __init__(self, input_dim, mask, hidden_dims, conditioning_size=None, single_function=True):
        super().__init__()
        ...
        self.s = [
            nn.Linear(input_dim + conditioning_size, hidden_dims[0]), nn.ReLU()
        ]
        ...
        ll = nn.Linear(hidden_dims[-1], input_dim)
...
class LinearNVP(nn.Module):
    """
        NVP model, alternating affine coupling layers
    ...
    @property
    def prior(self):
        return td.Normal(self.prior_mean, self.prior_var)
